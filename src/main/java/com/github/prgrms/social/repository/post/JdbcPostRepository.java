...
public class JdbcPostRepository implements PostRepository {
  ...
  public Optional<Post> findById(Id<Post, Long> postId /*추가로 필요한 인자들을 선언*/) {
    ...
    List<Post> results = jdbcTemplate.query(
      "SELECT p.*,u.email,u.name FROM posts p JOIN users u ON p.user_seq=u.seq WHERE p.seq=?",
      mapper,
      postId.value()
    );
  ...
  public List<Post> findAll(Id<User, Long> userId, /*추가로 필요한 인자들을 선언*/ long offset, int limit) {
    ...
    return jdbcTemplate.query(
      "SELECT p.*,u.email,u.name FROM posts p JOIN users u ON p.user_seq=u.seq WHERE p.user_seq=? ORDER BY p.seq DESC",
      mapper,
      userId.value()
    );
  }
